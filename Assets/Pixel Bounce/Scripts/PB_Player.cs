...
public class PB_Player:MonoBehaviour{
	...
	public void Slash(){										// Player melee attack
	...
	public void SlashToJumpAnim(){	// Delayed function to end Slash animation
		_anim.Play("Jump");
	}
	...
	public void Jump(float _amount,bool bounce,bool dive) {
		if(!_stunned && _alive){											// Player can not jump if stunned or dead
			...
			}else if(!_diving && transform.position.y > -.25f){				// Player dives if already jumping
				_anim.Play("Dive");
