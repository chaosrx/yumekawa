...
public class PB_Controller:MonoBehaviour{
	...
	public void DisableController(float seconds) {			// Disables controller for x amount of time
		if(_enabled){
			_enabled = false;
	...
	public void Update() {
		...
		foreach(Touch touch in Input.touches) {
			...
			if(touch.phase == TouchPhase.Ended && Physics.Raycast(ray,out hit)){		//When user stops touching screen but not pressing button (button just resets, nothing happens)
